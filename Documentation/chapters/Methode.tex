...
Intelligenz bestimmt (siehe \nameref{sub:t_ml_func}). Diese künstliche
...
handgeschriebene Ziffern erkennt (siehe \nameref{sub:m_eval_rec}). Diese zweite KI
beurteilt in jedem Step, welche Ziffern sie in der Vorlage und in der aktuellen  %TODO Code präzision
...
funktioniert die Reward-Function allerdings nicht nicht. Das heisst, der Agent
kann den akkumulierten Reward nicht vergrössern. Zwei Ansätze gehen auf dieses
Problem ein. Beide Ansätze sind Teil dieser Variation.

Der erste Ansatz schlägt vor, die zweite KI erst ab einer gewissen prozenutalen
Übereinstimmung (siehe \nameref{sub:m_eval_proc}) einzusetzen.Für diese
Variation löst die korrekte Erkennung erst ab einer prozentualen Übereinstimmung
von $20\%$ einen positiven Reward aus. Diese zusätzliche Bedingung ist
notwendig, weil die Beurteilungen der zweiten KI teilweise für einen
menschlichen Betrachter fragwürdig sin. Zum Beispiel schätzt die zweite KI eine
leere Zeichenfläche mit einer hohen Wahrscheinlichkeit als eine Eins ein. Das
ist ein Problem, weil dadurch der Agent einen positiven Reward für eine leere
Zeichenfläche erhält. Das stört das weitere Lernverhalten, weil es die
Wahrscheinlichkeit erhöht, dass die KI nicht mehr zeichnet

Der zweite Ansatz verwendet ebenfalls das Kriterium der prozentualen
Übereinstimmung. Dieses Kriterium ist in dieser Variation gleich wie in der
Basis Reward-Function (siehe \nameref{sub:m_var_base}) implementiert. Der
Unterschied liegt darin, dass in dieser Version der Reward, ausgelöst durch die
Basis Reward-Function, über das Training hinweg linear kleiner wird. Damit nimmt
die Relevanz der Basis Reward-Function kontinuierlich ab. Gleichzeitig nimmt der
Reward für eine korrekte Erkennung der Ziffer linear zu. Die Reward Function
ändert sich somit über den Verlauf des Trainings hinweg. Die folgende Abbildung
(autoref{decrementor graph}) stellt dar, wie sich die Grösse des Rewards der
beiden Reward-Functions, beschrieben durch einen Faktor, über das Training
hinweg verändert.

%todo Bild Decrementor

Das Zusammenspiel der beiden Reward-Functions hat den Vorteil, dass die
künstliche Intelligenz am Anfang des Trainings bereits für kleine Erfolge
positive Rewards erzielt. Die Reward-Function der Erkennbarkeit ermöglichst im
Gegensatz zu der Basis Reward-Function erst bei einer korrekten Erkennung einen
Reward. Diese korrekte Erkennung ist für eine untrainierte KI schwer zu
erreichen. Deswegen muss die KI durch die Basis Reward-Function gewissermassen
vortrainiert werden, um von der Reward-Function der Erkennbarkeit zu
profitieren.
