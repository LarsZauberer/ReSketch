\chapter{Methode}\label{chap:m}github ist ein
...
Variationen (siehe \doubleref{chap:m_var}). Die Variationen haben jeweils einen
unterschiedlichen Fokus auf die definierten Kriterien. Die Auswertung (siehe
\doubleref{chap:m_auswert}) bezieht sich ebenfalls auf die definierten Kriterien.
...
\doubleref{chap:m_eval}). Mit anderen Worten trainiert die KI auf diese Kriterien.
...
\doubleref{sub:t_rl_func}) stellen diese Eigenschaften bereit. Eine Reward
...
Keras Frameworks implementiert (siehe \doubleref{chap:t_ml}). 
...
(siehe \doubleref{sub:t_ver_dood}). Von Doodle-SDQ ist das neuronale Netz, bezogen
...
\doubleref{chap:t_ml}) und haben somit eine Grösse von $28\times28$ Pixeln. Die
...
\begin{figure}[!ht]
  ...
  \caption{Architektur des neuronalen Netzes im Grundprogramm (eigene Abbildung, mit Keras erstellt)}\label{fig:architecture}
\end{figure}
...
\doubleref{chap:m_var}) sind durch den Bayesian Optimization Algorithmus optimiert
(siehe \doubleref{sub:t_ml_hyper}). Die Implementierung des Algorithmus in Python
...
Testumgebung berechnet (siehe \doubleref{sub:m_auswert_test}). Auf welchem
...
Intelligenz bestimmt (siehe \doubleref{sub:t_ml_func}). Diese zweite KI beurteilt
...
Dieses Kriterium ist in der Fragestellung (siehe \doubleref{chap:einleit})
...
\doubleref{sub:m_eval_proc}) mindestens $70\%$ beträgt und die Zahl der Definition
entsprechend erkannt wird (siehe \doubleref{sub:m_eval_rec}). Wenn die Zeichnung
...
(siehe \doubleref{chap:m_grund}). Bei einigen dieser Variationen handelt es sich
um konkrete Implementierungen der definierten Kriterien in die Reward Function
(siehe \doubleref{sub:t_rl_func}). Die Reward Function kann dabei auch auf
...
(siehe \doubleref{chap:m_grund}) zu einer funktionierenden künstlichen
Intelligenz. Diese Reward Function implementiert das Kriterium der prozentualen
Übereinstimmung (siehe \doubleref{sub:m_eval_proc}). Der Reward für eine Action
...
\doubleref{sub:m_eval_speed}) kann in die Reward-Function integriert werden.
...
Reward-Function (siehe \doubleref{sub:m_var_base}). Die Anpassung davon sieht
...
begeht (siehe \doubleref{sub:m_grund_data}). $S$ entspricht der Anzahl Steps
...
Episode nicht fertigstellt, ist $f = 1$ (siehe \doubleref{sub:m_eval_speed}). In
...
zwischen verschiedenen Episodes im Replay-Buffer (siehe \doubleref{sub:t_rl_func}).
...
sich die Umgebung, in der sich der Agent bewegt (siehe \doubleref{sub:t_rl_func}).
...
allerdings mit den anderen Variationen (siehe \doubleref{sub:m_var_rec} und
\doubleref{sub:m_var_speed}) kompatibel, Da diese ausschliesslich die
...
Beschleunigungsvektor. Der Action-Space (siehe \doubleref{sub:t_rl_func}) besteht
...
haben dabei eine Länge von $1.8$ Pixeln (siehe \autoref{fig:physics-actionspace}). % TODO: Werte nicht mehr ganz korrekt, wegen Hyperparameter optimierung
...
der gewählten Action, in jedem Step um $0.2$ Pixel verringert. Das simuliert eine  % TODO: Faktor Hyperparameter optimierung
...
\doubleref{sub:t_rl_func}) ist. Der Agent berücksichtigt deswegen seine
Geschwindigkeit nicht in seinen Entscheidungen. Die Lösung dieses Problems
bietet eine Verschiebung des Local image patch (siehe \doubleref{sub:t_ver_dood}).
...
\begin{figure}[!ht]
  ...
  \caption{Beispiele der verwendeten Motiven aus dem QuickDraw Datenset}\label{fig:quickdraw-examples}
\end{figure}
