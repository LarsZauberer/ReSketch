...
kombinierbar. Eine der Variationen (siehe \doubleref{sub:m_var_phy}) ist auf
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=\textwidth-6cm]{images/methode/speedplot.png}
...
Policy zur Wahl der Actions zu verwenden (siehe \doubleref{sub:t_rl_func}). Die
...
der höchsten Q-Values (siehe \doubleref{sub:t_rl_func}) auswählen. Ohne diese
...
akkumulierten Reward erreicht und dadurch auch keine guten Leistungen erzielt.
...
\doubleref{sub:r_tab_nachzeich}). Die Softmax Temperatur wird über das Training
...
\doubleref{chap:m_eval}). Die übrigen Kriterien sind ohne eine
...
Wie im Test der nachzeichnenden KI (siehe \doubleref{sub:m_auswert_nachzeich}),
