\chapter{Methode}
Das Ziel der Methode ist es, die in der Fragestellung beschriebene künstliche
Intelligenz zu entwickeln. Die Auswertung und die Diskussion der Leistung dieser
Künstlichen Intelligenz führt schlussendlich zu einer Antwort auf die
Fragestellung. Die Entwicklung der künstlichen Intelligenz selbst besteht aus
zwei Teilen. Im einen Teil werden die Kriterien definiert, nach denen die
Leistung der künstlichen Intelligenz evauliert wird. Im anderen Teil wird die
künstliche Intelligenz zusammen mit verschiedenen Variationen davon entwickelt.
Die verschiedenen Variationen haben jeweils einen anderen Fokus auf die
definierten Kriterien.

\section{Evaluierung der Leistung}
In diesem Kapitel sind die Kriterien definiert, die die Leistung der künstlichen
Intelligenz evaluierenwir. Die Kriterien erfassen unterschiedliche Aspekte der
Fragestellung (siehe Diskussion). Es gibt drei Kategorien von Kriterien: messbare % TODO: Ref setzen
Kriterien, nicht messbare Kriterien und teilweise messbare Kriterien.
...
Ein Beispiel für ein messbares Kriterium ist die \emph{prozentuale Genauigkeit}
der Zeichnung. Spezifischer ist damit die prozentuale Übereinstimmung der
weissen (gezeichneten) Pixel zwischen der Vorlage und der Zeichnung gemeint. Der Wert $K$ dieses
Kriteriums zu einem bestimmten Schritt $t$ berechnet sich aus folgender Formel:
\begin{equation}
  \label{eq:m_reward}
  K(t) = \frac{G(t)}{G_{max}}
\end{equation}
