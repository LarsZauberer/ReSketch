...
welche Ziffer es sich handelt. Das heisst, der Output des
Computerprogramms soll der Ziffer entsprechen, die auf dem Bild des Inputs zu sehen
...
basierend auf dem Beispielproblem aus dem letzten Abschnitt (siehe \doubleref{chap:t_ml}).
...
Datenset ist labeled (siehe \doubleref{chap:t_ml}). Falls die Beurteilung des
...
selbst anpassen, um die Beurteilungen zu verbessern. Diese Eigenschaften sind in künstlichen neuronalen Netzen
(siehe \doubleref{sub:t_ml_nn}) vorhanden, wodurch diese in
Machine Learning Modellen Anwendung finden.
...
Learning und wird häufig für \emph{Hidden Layers} verwendet (source...). Softmax wird im   %Todo: Quelle
Vergleich meistens für die Activation Function des \emph{Output Layers}                   %Todo: Quelle
...
Beispielproblem (siehe \doubleref{chap:t_ml}) bestände der Input Layer aus
...
\doubleref{sub:t_ml_func}) einzelne Gewichte im neuronalen Netz an, in der Hoffnung,
...
Modells (siehe \doubleref{sub:t_ml_func}). Die inneren, nicht betrachteten
...
Supervised Learning und Unsupervised Learning (siehe \doubleref{chap:t_ml}) sind
...
indem es durch Beurteilungen (siehe \doubleref{sub:t_ml_func}) mit dieser
...
\doubleref{sub:t_ml_nn}). Der Input in dieses neuronale Netz ist die Observation
...
Die schlussendliche Entscheidung der auszuführenden Aktion wird durch eine
\emph{Policy} bestimmt. Softmax und Epsilon Greedy sind 2 Beispiele für eine
Policy \cite[S. 34]{sutton_reinforcement_2014}. Die Epsilon-Greedy Strategie
sieht vor, dass die Entscheidung mit einer Wahrscheinlichkeit von $\epsilon$
(Epsilon) auf eine zufällige Action fällt. 

Ansonsten fällt die Entscheidung auf
diejenige Action mit dem höchsten Q-Value. Durch die zufälligen Actions erkundet
der Agent die Umgebung, in der Hoffnung, auf bessere Optionen für zukünftige
Entscheidungen zu stossen. \cite{rajendra_koppula_exploration_nodate}.

Die Softmax Policy wählt Aktionen auf Basis einer Wahrscheinlichkeitsverteilung.
Jede Aktion wird also mit einer gewissen Wahrscheinlichkeit ausgeführt. Die
Wahrscheinlichkeitsverteilung berechnet sich aus der Softmax Activation Function
von dem Output Layer des neuronalen Netzes.
...
spezifischer durch Deep Q-Learning (siehe \doubleref{sub:t_rl_func}) erlernt,
...
\doubleref{sub:t_rl_func}) ist in zwei Teile gegliedert: der Global Stream und
...
von $28\times28$ Pixeln haben. Dabei beschreibt eine rationale Zahl den Wert von
...
eine Concatenation Layer (siehe \doubleref{sub:t_ml_nn}) zusammengeführt.
