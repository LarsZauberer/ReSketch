...
\cite{noauthor_what_nodate} Mit Maschinen und Systemen sind in den allermeisten    %Todo Quelle Oxford Dict
...
\cite{laurenz_wuttke_was_2021-1}. Die Zielvariable muss dabei im Voraus für jeden   % Todo Quelle Paper supervised
...
diesem Fall um das MNIST Datenset \cite{yann_lecun_mnist_nodate}. Dieses Datenset  % Todo quelle new date
...
\cite{yann_lecun_papers_nodate}. Jedes Bild hat eine Auflösung von $28\times28$  % todo quelle new paper
...
\cite{noauthor_artificial_nodate}. Diese Arbeit behandelt künstliche neuronale % todo neue Quelle towards data science (differences)
...
Learning und wird häufig für \emph{Hidden Layers} verwendet \cite{a}. Softmax wird im   %Todo: Quelle Relu
Vergleich meistens für die Activation Function des \emph{Output Layers}                   %Todo: Quelle softmax activation
verwendet \cite{b}. Diese Begriffe werden im nächsten Abschnitt genauer erläutert.
...
hervorzuheben \cite{deshpande_beginners_nodate}. Concatenation Layers   % Todo Quelle CNN
...
\cite{david_e_rumelhart_learning_nodate}. % Todo Quelle Name Anpassen
...
15]{garnett_bayesian_nodate}. Eine Black Box ist ein häufig komplexes System,        % Todo Quelle datum anpassen
...
Theorem \cite[S. 7]{garnett_bayesian_nodate}. Diejenigen Parameter, die den     % Todo Quelle datum anpassen
...
Die Epsilon-Greedy Policy sieht vor, dass die Entscheidung mit einer
...
\cite{rajendra_koppula_exploration_nodate}. Die Softmax Policy wählt Actions auf % Todo neue Quelle Coggan
Basis einer Wahrscheinlichkeitsverteilung. Jede Action wird also mit einer  %Todo Quelle Softmax policy
