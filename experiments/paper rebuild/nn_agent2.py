...
class DeepQNetwork(object):
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            self.global_input = tf.compat.v1.placeholder(tf.float32, shape=[None, *self.global_input_dims], name='global_inputs')
            self.local_input = tf.compat.v1.placeholder(tf.float32, shape=[None, *self.local_input_dims], name='local_inputs')
            self.q_target = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_actions], name='q_value')

            #global convolution
            glob_conv1 = tf.compat.v1.layers.conv2d(inputs=self.global_input, filters=32, kernel_size=(8,8), strides=4, name='glob_conv1', padding="SAME", 
                                    kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            glob_conv1_activated = tf.nn.relu(glob_conv1)

            glob_conv2 = tf.compat.v1.layers.conv2d(inputs=glob_conv1_activated, filters=64, kernel_size=(4,4), strides=2, name='glob_conv2', padding="SAME",
                                    kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            glob_conv2_activated = tf.nn.relu(glob_conv2)

            glob_conv3 = tf.compat.v1.layers.conv2d(inputs=glob_conv2_activated, filters=64, kernel_size=(3,3),strides=1, name='glob_conv3', padding="SAME",
                                    kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            glob_conv3_activated = tf.nn.relu(glob_conv3)

            #local convolution
            loc_conv1 = tf.compat.v1.layers.conv2d(inputs=self.local_input, filters=128, kernel_size=(11,11), strides=1, name="loc_conv1", padding="SAME",
                                    kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            ...
            dense1 = tf.compat.v1.layers.dense(fcl_input, units=self.fc1_dims, activation=tf.nn.relu,
                                    kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            ...
            self.loss = tf.reduce_mean(input_tensor=tf.square(self.Q_values - self.q_target))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size, global_input_dims, local_input_dims,
